...
package cursotalend.job_caga_vendas_0_1;
...
/**
 * Job: job_caga_vendas Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class job_caga_vendas implements TalendJob {
	...
	private final String jobName = "job_caga_vendas";
	private final String projectName = "CURSOTALEND";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					job_caga_vendas.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(job_caga_vendas.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class saida_vendasStruct implements routines.system.IPersistableRow<saida_vendasStruct> {
		final static byte[] commonByteArrayLock_CURSOTALEND_job_caga_vendas = new byte[0];
		static byte[] commonByteArray_CURSOTALEND_job_caga_vendas = new byte[0];
		...
		public Integer Ano_venda;

		public Integer getAno_venda() {
			return this.Ano_venda;
		}

		public Integer Mes_venda;

		public Integer getMes_venda() {
			return this.Mes_venda;
		}

		public Integer Dia_venda;

		public Integer getDia_venda() {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_CURSOTALEND_job_caga_vendas.length) {
					if (length < 1024 && commonByteArray_CURSOTALEND_job_caga_vendas.length == 0) {
						commonByteArray_CURSOTALEND_job_caga_vendas = new byte[1024];
					} else {
						commonByteArray_CURSOTALEND_job_caga_vendas = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_CURSOTALEND_job_caga_vendas, 0, length);
				strReturn = new String(commonByteArray_CURSOTALEND_job_caga_vendas, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_CURSOTALEND_job_caga_vendas) {

				try {

					int length = 0;

					this.id_venda = readInteger(dis);

					this.cod_vendedor = readInteger(dis);

					this.cod_produto = readInteger(dis);

					this.cod_departamento = readInteger(dis);

					this.Ano_venda = readInteger(dis);

					this.Mes_venda = readInteger(dis);

					this.Dia_venda = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.id_venda, dos);

				// Integer

				writeInteger(this.cod_vendedor, dos);

				// Integer

				writeInteger(this.cod_produto, dos);

				// Integer

				writeInteger(this.cod_departamento, dos);

				// Integer

				writeInteger(this.Ano_venda, dos);

				// Integer

				writeInteger(this.Mes_venda, dos);

				// Integer

				writeInteger(this.Dia_venda, dos);
				...
				writeInteger(this.num_nota, dos);
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("id_venda=" + String.valueOf(id_venda));
			sb.append(",cod_vendedor=" + String.valueOf(cod_vendedor));
			sb.append(",cod_produto=" + String.valueOf(cod_produto));
			sb.append(",cod_departamento=" + String.valueOf(cod_departamento));
			sb.append(",Ano_venda=" + String.valueOf(Ano_venda));
			sb.append(",Mes_venda=" + String.valueOf(Mes_venda));
			sb.append(",Dia_venda=" + String.valueOf(Dia_venda));
			sb.append(",dtc_venda=" + dtc_venda);
			sb.append(",qtd_venda=" + String.valueOf(qtd_venda));
			sb.append(",val_venda=" + val_venda);
			sb.append(",num_nota=" + String.valueOf(num_nota));
			sb.append("]");
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		...
		public Integer cod_gerencia;

		public Integer getCod_gerencia() {
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_CURSOTALEND_job_caga_vendas) {
				...
				try {
					...
					this.cod_cliente = readInteger(dis);
					...
					this.cod_cidade = readInteger(dis);
					...
					this.cod_marca = readInteger(dis);
					...
					this.cod_gerencia = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.cod_cliente, dos);
				...
				writeInteger(this.cod_cidade, dos);
				...
				writeInteger(this.cod_marca, dos);
				...
				writeInteger(this.cod_gerencia, dos);
	...
	public void tFileInputExcel_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "saida_vendas");
				}
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:73bp1d/5tMcjooptUYwGalQ/1pIvYaHfB8KSGDKD7vUucAZXjhM=");
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE " + tableName_tDBOutput_1
							+ "(id_venda INT ,cod_vendedor INT ,cod_produto INT ,cod_departamento INT ,Ano_venda INT ,Mes_venda INT ,Dia_venda INT ,dtc_venda VARCHAR2(10)  ,qtd_venda INT ,val_venda VARCHAR2(7)  ,num_nota INT )");
				}
				String insert_tDBOutput_1 = "INSERT INTO " + tableName_tDBOutput_1
						+ " (id_venda,cod_vendedor,cod_produto,cod_departamento,Ano_venda,Mes_venda,Dia_venda,dtc_venda,qtd_venda,val_venda,num_nota) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
...
// ###############################
				...
				Object source_tFileInputExcel_1 = "C:/Users/wise/Desktop/talend/datasets/Notas Fiscais.xls";
				...
				try {
					...
					sheetList_tFileInputExcel_1.addAll(
							regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, "Nota Fiscal", false));
...
//end%>
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.nom_cliente = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.nom_cliente = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.nom_vendedor = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.nom_vendedor = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.nom_cidade = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.nom_cidade = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.cod_produto = ParserUtils
											.parseTo_Integer(temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1]);
								} else {
									row1.cod_produto = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.Nom_produto = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.Nom_produto = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.Nom_marca = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.Nom_marca = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.cod_departamento = ParserUtils
											.parseTo_Integer(temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1]);
								} else {
									row1.cod_departamento = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.nom_departamento = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.nom_departamento = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.cod_gerencia = ParserUtils
											.parseTo_Integer(temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1]);
								} else {
									row1.cod_gerencia = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.nom_gerencia = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.nom_gerencia = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.dtc_venda = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.dtc_venda = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.val_venda = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.val_venda = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.Latitude = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.Latitude = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.Longitude = temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1];
								} else {
									row1.Longitude = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
...
// # Output table : 'saida_vendas'
									saida_vendas_tmp.id_venda = row1.id_venda;
									saida_vendas_tmp.cod_vendedor = row1.cod_vendedor;
									saida_vendas_tmp.cod_produto = row1.cod_produto;
									saida_vendas_tmp.cod_departamento = row1.cod_departamento;
									saida_vendas_tmp.Ano_venda = row1.Ano_venda;
									saida_vendas_tmp.Mes_venda = row1.Mes_venda;
									saida_vendas_tmp.Dia_venda = row1.Dia_venda;
									saida_vendas_tmp.dtc_venda = row1.dtc_venda;
									saida_vendas_tmp.qtd_venda = row1.qtd_venda;
									saida_vendas_tmp.val_venda = row1.val_venda;
									saida_vendas_tmp.num_nota = row1.num_nota;
...
// Start of branch "saida_vendas"
								if (saida_vendas != null) {
									...
									if (execStat) {
										runStat.updateStatOnConnection(iterateId, 1, 1, "saida_vendas");
									}
...
// ###############################      
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "saida_vendas");
				}
	...
	public static void main(String[] args) {
		final job_caga_vendas job_caga_vendasClass = new job_caga_vendas();

		int exitCode = job_caga_vendasClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = job_caga_vendas.class.getClassLoader()
					.getResourceAsStream("cursotalend/job_caga_vendas_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = job_caga_vendas.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : job_caga_vendas");
		}
...
/************************************************************************************************
 * 78637 characters generated by Talend Open Studio for Big Data on the 2 de
 * Abril de 2022 17h22min34s BRT
 ************************************************************************************************/